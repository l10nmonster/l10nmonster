...
var FsSource = class {
  ...
  async fetchResourceStats() {
    const resources = [];
    const expandedFileNames = globbySync(this.globs.map((g) => path7.join(this.baseDir, g)));
...
async function createMonsterManager({ configPath, options, logger: logger2, env }) {
  ...
  let prj = options.prj;
  prj && (prj = prj.split(","));
  const opsMgr = Config.opsDir ? new OpsMgr({ opsDir: path8.join(baseDir, Config.opsDir), logger: logger2 }) : new OpsMgr({ logger: logger2 });
  helpers.mergeProps({
    baseDir,
    opsMgr,
    env: env ?? {},
    arg: options.arg,
    logger: logger2 ?? { verbose: () => false, info: () => false, warn: () => false, error: () => false },
    regression,
    prj
  });
  try {
...
function withMonsterManager(configPath, cb) {
  const l10nmonsterCfg = import_vscode.default.workspace.getConfiguration("l10nmonster");
...
var L10nMonsterViewTreeDataProvider = class {
...
function getMonsterPage(title, body) {
  return `
    <!DOCTYPE html>
    <html lang="en">
        <head>
...
function getPrintable(plain, structured, inflight) {
  ...
  if (structured) {
    return structured.map((part) => typeof part === "string" ? clean(part) : `<b>${clean(part.v)}</b>`).join("");
  }
  if (plain) {
    return clean(plain);
  }
  return inflight ? "\u{1F680}" : "\u274C";
}
async function viewJob(jobGuid, hasRes) {
  return withMonsterManager(this.configPath, async (mm) => {
    ...
    const panel = import_vscode2.default.window.createWebviewPanel(
      "jobView",
      `Job: ${jobGuid}`,
      import_vscode2.default.ViewColumn.One,
...
async function fetchAnalyzePanel(mm) {
  const analyzePanel = Object.entries(mm.analyzers).map(([name, analyzer]) => ({
    key: name,
    iconPath: import_vscode3.default.ThemeIcon.File,
...
async function runAnalyzer(name, helpParams) {
  return withMonsterManager(this.configPath, async (mm) => {
    const Analyzer = mm.analyzers[name];
    let params;
    if (helpParams && (params = await import_vscode3.default.window.showInputBox({ placeHolder: helpParams })) || !helpParams) {
      const analysis = await analyzeCmd(mm, Analyzer, params?.split(" ") || []);
      const panel = import_vscode3.default.window.createWebviewPanel(
        "analyzerView",
        `${name} ${params ?? ""}`,
        import_vscode3.default.ViewColumn.One,
        { enableFindWidget: true }
      );
      panel.webview.html = getMonsterPage(name, `
                <h2>Analyzer: ${name} ${helpParams ? params : ""}</h2>
                ${analysis.head ? analysis.body.length > 0 ? `<table>
                        <tr>${analysis.head.map((col) => `<th>${col}</th>`).join("\n")}</tr>
                        ${analysis.body.map((row) => `<tr>${row.map((col) => `<td>${col}</td>`).join("")}</tr>`).join("\n")}
                        </table>` : "<h4>Nothing found!</h4>" : `<pre>${analysis.body}</pre>`}
            `);
    }
  });
}
...
async function initL10nMonster(context) {
  ...
  if (configPath && (0, import_fs10.existsSync)(configPath)) {
    ...
    return withMonsterManager(configPath, async (mm) => {
      ...
      import_vscode4.default.window.registerTreeDataProvider("jobsView", jobsViewProvider);
      ...
      context.subscriptions.push(import_vscode4.default.commands.registerCommand("l10nmonster.runAnalyzer", (name, helpParams) => analyzeViewProvider.runAnalyzer(name, helpParams)));
      import_vscode4.default.window.registerTreeDataProvider("analyzeView", analyzeViewProvider);
      return true;
    });
  }
  logger.error(`Could not find L10n Monster config at: ${configPath}`);
  await import_vscode4.default.commands.executeCommand("setContext", "l10nMonsterEnabled", false);
  return false;
}
async function l10nmanagerCommand(context) {
  const status = await initL10nMonster(context);
  import_vscode4.default.window.showInformationMessage(status ? "L10n Monster re-initialized" : "Problems initializing L10n Monster");
  status && await import_vscode4.default.commands.executeCommand("statusView.focus");
}
async function activate(context) {
  logger.info(`L10n Monster Manager is now active!`);
  context.subscriptions.push(import_vscode4.default.commands.registerCommand("l10nmonster.l10nmanager", l10nmanagerCommand));
